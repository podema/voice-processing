...
int main(int argc, const char *argv[]) {
  ...
  gmm.vq_lbg(data,nmix,ending_iterations,threshold,verbose);

  //Apply EM to estimate GMM parameters (complete the funcion in gmm.cpp)
...
int read_options(int ArgC, const char *ArgV[], Directory &input_dir, Ext &input_ext, vector<string> &filenames, 
  ...
  if (!input_ext.empty() && input_ext[0] != '.') input_ext = '.' + input_ext;
