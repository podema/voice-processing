...
float verify(const vector<GMM> &vgmm,unsigned int u,const fmatrix &dat) {
...
int read_options(int ArgC, const char *ArgV[], vector<Directory> &input_dirs, vector<Ext> &input_exts,
	...
	for (unsigned int i=0; i<input_dirs.size(); ++i) {
		if (!input_dirs[i].empty() && *(input_dirs[i].rbegin()) != '/') input_dirs[i] += '/';
		if (!gmm_dirs[i].empty() && *(gmm_dirs[i].rbegin()) != '/') gmm_dirs[i] += '/';
		if (!input_exts[i].empty() && input_exts[i][0] != '.') input_exts[i] = '.' + input_exts[i];
		if (!gmm_exts[i].empty() && gmm_exts[i][0] != '.') gmm_exts[i] = '.' + gmm_exts[i];
	}
	...
	is.open(ArgV[2]);
	if (!is.good()) {
		cerr << "ERROR opening list of user candidates: " << ArgV[2] << endl;
		return -6;
	}
	while (is >> s)
...
int main(int argc, const char *argv[]) {
	...
	int retv = read_options(argc, argv, input_dirs, input_exts,gmm_dirs, gmm_exts, input_filenames, gmm_filenames,candidates);
	...
	if (input_filenames.size() != candidates.size()) {
		cerr << "Error: num_candidates != num_files\n";
	}
	...
	for (unsigned int i=0; i<input_filenames.size(); ++i) {
		...
		unsigned int aux;
		for(unsigned int j=0;j<gmm_filenames.size();++j){
			if(candidates[i]==gmm_filenames[j]){aux=j; break;}
		}
		cout<<input_filenames[i]<<'\t'<<candidates[i]<<'\t'<<verify(vgmm,aux,dat)<<endl;
	}
