...
int GMM::vq_maximization(const fmatrix &data, const ivector &indexes, int update_variances) {
...
int GMM::vq_lloyd(const fmatrix &data, unsigned int max_it, float inc_threshold, int verbose) {
  ...
  while (1) {
    ...
    if (((iteration > 0) && (inc < inc_threshold)) || (++iteration == max_it)) {
      vq_maximization(data, indexes, 0);
      break;
    } else {
      vq_maximization(data, indexes, 0);
      old_dist = new_dist;
