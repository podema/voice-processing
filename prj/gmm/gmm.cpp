...
/*
  // Working directly with probabilities can give numerical problems; 
  // It is better to work with logprob (see below)

  float gaussian_prob(unsigned int vector_size, const float *mu, const float *inv_sigma, const float *x) {
...
int GMM::em(const fmatrix &data, unsigned int max_it, float inc_threshold, int verbose) {
	...
	for (iteration=0; iteration<max_it; ++iteration) {
		...
		///TODO: loop, em_expectation + em_maximization
		///Stop if the prob. does not increases more than inc_threshold
...
int GMM::split(unsigned int target_size) {
	...
	if (2*nmix <= target_size) {
		...
		/* TO DO: select mixtures with larger variance (now, the first ones) */
		for (i=old_size, j=0; i<nmix; ++i, ++j)
			split_mixture(j, i);
	}
	return nmix;
